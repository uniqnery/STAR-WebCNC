...
- **로고 위치**: **좌측 사이드바(메뉴바) 최상단**에 고정 배치
- **호기 선택기 위치**: **Machines 하위 페이지(Remote Panel, Scheduler, Transfer)의 페이지 헤더**에 제목 옆 배치
  - 사이드바에서는 호기 선택기 제거 (v3.2)
  - 각 페이지 진입 시 페이지 제목 옆 드롭다운으로 장비 선택
- **제어/사용자 버튼 노출 정책**:
  - **제어실행요청** 및 **로그인 사용자 표시**는 제어가 수반되는 특정 화면에서만 상단 바(Header)에 표시
  - 대상 화면: **Remote Panel**, **Scheduler**
...
- **호기 선택 동작 원칙**:
  ...
  - 선택된 호기는 Zustand store에 저장되어 모든 Machines 하위 페이지에서 공유

```
┌──────────────────────────────────────┐ ┌────────────────────────────────────┐
│ [메뉴바 상단]                        │ │ [페이지 헤더 - Machines 하위 전용] │
│  [STAR 로고]                         │ │  원격 제어  [▼ 1호기 자동선반]    │
│                                      │ │  스케줄러   [▼ 전체 장비]         │
│                                      │ │  프로그램 전송 [▼ 장비 선택]      │
└──────────────────────────────────────┘ └────────────────────────────────────┘
```
...
│ │       · 실시간 가동 현황 한눈에 확인                          │  │
│ │       · 장비 클릭 → Remote Panel 이동 + 해당 호기 선택        │  │
│ └────────────────────────────────────────────────────────────────┘  │
└─────────────────────────────────────────────────────────────────────┘
     │
     ├─────────────────────────────────────────────────────────────────┐
...
│                                                                      │
├─ [ Machines 메뉴 (펼침/접힘) ] ──────────────────────────────────────│
│   │  ※ 기본 접힘 상태, 클릭 시 하위 메뉴 펼침                       │
│   │  ※ 하위 페이지 진입 시 헤더에 장비 선택 드롭다운 표시           │
│   │                                                                  │
│   ├─ [ Remote Panel (/remote) ]             [관리자/AS + 제어권 필요] │
│   │       │                                                          │
│   │       ▼                                                          │
│   │   ┌─────────────────────────────────────────────────────────┐    │
│   │   │ 원격 제어 [▼ 호기 선택]                                 │    │
...
│   │   │ ※ 장비 미선택 시: MachineCard 목록 표시 (카드 클릭 선택)│    │
│   │   │                                                         │    │
│   │   │ ┌───────────────────────┬─────────────────────────────┐ │    │
│   │   │ │ [좌측: 장비 상세]     │ [우측: 가상 조작반]         │ │    │
│   │   │ │ ─────────────────     │ ─────────────────────────── │ │    │
...
│   │   └─────────────────────────────────────────────┘                │
│   │                                                                  │
│   ├─ [ File Transfer (/transfer) ]          [관리자/AS 전용]         │
│   │       │                                                          │
│   │       ▼                                                          │
│   │   ┌─────────────────────────────────────────────┐                │
│   │   │ 프로그램 전송 [▼ 호기 선택]                │                │
...
│   └─────────────────────────────────────────────┘                    │
│                                                                      │
├─ [ Settings (/settings) ]                   [모든 사용자 조회 / 관리자 편집]
│        │                                                             │
│        ▼                                                             │
│   ┌─────────────────────────────────────────────┐                    │
│   │ 시스템 설정 (Settings)                      │                    │
...
| **POP (전체)** | ✅ 조회 | ✅ 조회 | ✅ 조회 |
| **MES (전체)** | ✅ 조회 | ✅ 조회 + 편집 | ✅ 전체 |
| **Settings** | ✅ 조회만 | ✅ 조회 + 편집 (카메라 ON/OFF, CRUD) | ✅ 전체 |
...
전체 프론트엔드는 **TailwindCSS `dark:` 프리픽스**를 활용한 다크 테마를 지원:

| 영역 | 라이트 | 다크 |
|------|--------|------|
| 페이지 배경 | `bg-gray-100` | `bg-gray-900` |
| 카드/패널 | `bg-white` | `bg-gray-800` |
| 테이블 행 hover | `hover:bg-gray-50` | `hover:bg-gray-700` |
| 텍스트 | `text-gray-900` | `text-gray-100` |
| 보더 | `border-gray-200` | `border-gray-700` |

**적용 범위**: NCMonitor, Transfer, Scheduler 등 장비 관련 페이지는 다크 테마 우선, Dashboard는 라이트/다크 모두 지원

#### 1.7.2 공장 레이아웃 편집 (Admin Only)

Dashboard의 FactoryView에서 **관리자 전용 편집 모드**를 지원:

```
packages/web/src/stores/layoutStore.ts
```

**아이템 타입**: `machine | barfeeder | conveyor | robot | table | pillar | wall | door | corridor | custom`

**기능**:
- 드래그앤드롭 위치 이동
- 아이템 회전/크기 조절
- 캔버스 줌/팬 (모바일 터치 대응)
- `localStorage` 영속화 (`zustand/middleware`)
- 장비 아이템은 텔레메트리 상태에 따라 색상 자동 변경 (online=초록, alarm=빨강, offline=회색)

#### 1.7.3 역할 기반 UI 제어

```typescript
// 쓰기 권한 체크 (공통)
const canEdit = user?.role === 'ADMIN' || user?.role === 'AS';
...
| 패턴 | 적용 위치 |
|------|----------|
| `canEdit` (ADMIN + AS) | 옵셋 편집, 카운터 편집, Transfer 실행 |
| `isAdmin` (ADMIN only) | 이력 삭제, 레이아웃 편집, 카메라 설정 |
...
### 2.6 Frontend 상태관리 패턴

#### 2.6.1 Zustand Store 구성

```
packages/web/src/stores/
├── machineStore.ts    # 장비/텔레메트리/스케줄러/NC데이터/DNC설정
├── fileStore.ts       # 파일관리 (Repository, CNC, 전송큐, GCode뷰어)
├── authStore.ts       # 인증 (사용자, 역할)
├── cameraStore.ts     # 카메라 설정 (ON/OFF, CRUD, localStorage)
└── layoutStore.ts     # 공장 레이아웃 편집기 (아이템, 편집모드)
```

**공통 패턴**:
- 모든 Store는 Zustand `create()` 기반
- `localStorage`를 통한 클라이언트 상태 영속화 (cameraStore, layoutStore, dncConfig)
- 머신별 데이터는 `Record<machineId, T>` 구조로 관리

#### 2.6.2 Custom Selector Hook 패턴

머신별 데이터를 구독할 때 **Custom Selector Hook**을 사용하여 불필요한 리렌더를 방지:

```typescript
// 사용 패턴 - machineStore.ts
export const useMachineTelemetry = (machineId: string) =>
  useMachineStore(state => state.telemetryMap[machineId]);

export const useFocasEvents = (machineId: string) =>
  useMachineStore(state => state.focasEvents[machineId] || []);

export const useSchedulerJobs = (machineId: string) =>
  useMachineStore(state => state.schedulerJobs[machineId] || []);

// fileStore.ts - path-aware selector
export const useRepoFiles = (machineId: string, pathKey: string) =>
  useFileStore(state => state.repoFiles[`${machineId}:${pathKey}`] || []);
```

**규칙**:
- Hook명: `useXxx` (React Hook 컨벤션)
- 빈 배열 fallback: `|| []`로 undefined 방지
- 스코프: `machineId` 또는 `machineId:pathKey`로 데이터 격리

#### 2.6.3 Mock 데이터 전략 (Phase 5 이전)

현재 개발 단계에서 모든 Store는 **Mock 데이터를 직접 로드**하며, API 실패 시 Mock fallback을 적용:

```
데이터 로드 흐름:
  Store Action 호출
    → API fetch 시도 (api.ts 스텁)
    → catch → Mock 데이터 반환
    → Store state 업데이트
```
...
| Store | Mock 데이터 |
|-------|------------|
| machineStore | 내장 (MOCK_MACHINES, MOCK_TELEMETRY 등) |
| fileStore | `components/filemanager/mockFileData.ts` |
| cameraStore | 내장 (MOCK_CAMERAS) |
| ncmonitor 뷰 | `components/ncmonitor/mockData.ts` |

**Phase 5 전환 계획**: Mock fallback → 실제 API 호출로 점진적 교체 (API 스텁은 `lib/api.ts`에 사전 정의)
...
│                                              ● 누구누구님이 사용중입니다    │
├─────────────────────────────────────────────────────────────────────────────┤
│ [도어록] [메모리모드] [바피더오토] [절삭유ON] [머시닝모드] [절단모드] [  ] [  ]│
├────────────────────────────────────┬────────────────────────────────────────┤
│ [NC 모니터 - Path1/Path2]          │ [스케줄러 테이블]                      │
│ ┌────────────────┬────────────────┐│ 현재: O0001/O9001  [▶실행] [■정지]    │
│ │ Path1 프로그램 │ Path2 프로그램 ││ ┌────────┬────────┬──────┬─────┬────┬──┐│
│ │ N0100 G00 X50  │ N0050 G00 X80  ││ │메인PGM │서브PGM │PRESET│COUNT│상태│삭││
│ │ N0101 G01 X25  │ N0051 G01 Z-25 ││ ├────────┼────────┼──────┼─────┼────┼──┤│
│ │ N0102 Z-30     │ N0052 X70      ││ │O0001   │O9001   │  100 │  87 │실행│ X││
│ │ ...6줄 표시    │ ...6줄 표시    ││ │O0002   │O9002   │   50 │   0 │대기│ X││
│ ├────────────────┴────────────────┤│ │O0003   │O9003   │  200 │   0 │대기│ X││
│ │ Path1 좌표 (절대/기계/잔량)     ││ │        [+ 행 추가]              │  ││
│ │ X1: 25.000 / 225.000 / 5.000    ││ └────────────────────────────────────┘│
│ │ Y1: 0.000  / -200.000 / 0.000   ││                                        │
│ │ Z1: -15.000 / 185.000 / 15.000  ││                                        │
...
├─────────────────────────────────────┴────────────────────────────────────────┤
│ [FOCAS 이벤트 로그]                                                          │
│ 10:15:32 📄 프로그램 O0020으로 변경되었습니다                                │
│ 10:15:33 ▶ 사이클스타트가 전송되었습니다                                     │
│ 10:15:34 ▶ 사이클스타트가 실행되었습니다                                     │
│ 10:20:45 ✓ 카운트가 완료되었습니다 (87/100)                                  │
└─────────────────────────────────────────────────────────────────────────────┘
```
...
| 영역 | 설명 |
|------|------|
| **헤더** | 스케줄러 제목 + 장비 선택 드롭다운 + 제어권 획득 버튼 + 상태 메시지 |
| **인터록 바** | 도어록, 메모리모드, 바피더오토, 절삭유ON, 머시닝모드, 절단모드, 기타(2) |
| **DNC 경로 설정** | 접힘/펼침 패널, Path별 DNC 프로그램 경로 설정 (인터록 바 아래) |
| **NC 모니터** | Path1/Path2 프로그램 내용, 좌표계(절대/기계/잔량), 스핀들/이송 정보, 카메라 탭 |
| **스케줄러 테이블** | 현재 실행 작업 + 대기 작업 목록 (메인PGM, 서브PGM, PRESET, COUNT, 상태, 삭제) |
| **이벤트 로그** | FOCAS 원격 명령 및 상태 피드백 실시간 모니터링 |

**Path1/Path2 데이터 구조** (CNC 자동선반):
- Path1: X1, Y1, Z1, C1 축
- Path2: X2, Y2, Z2, C2 축
- 각 Path별 좌표계: 절대좌표 / 기계좌표 / 잔량(남은거리)

**FOCAS 이벤트 타입**:

| 이벤트 타입 | 설명 | 예시 메시지 |
|-------------|------|-------------|
| `PROGRAM_SELECT` | 프로그램 선택/변경 | "프로그램 O0020으로 변경되었습니다" |
| `CYCLE_START` | 사이클 스타트 전송 | "사이클스타트가 전송되었습니다" |
| `CYCLE_START_ACK` | 사이클 스타트 실행 확인 | "사이클스타트가 실행되었습니다" |
| `FEED_HOLD` | 피드 홀드 | "피드홀드가 실행되었습니다" |
| `RESET` | 리셋 | "리셋이 실행되었습니다" |
| `ALARM_CLEAR` | 알람 해제 | "알람이 해제되었습니다" |
| `MODE_CHANGE` | 모드 변경 | "모드가 AUTO로 변경되었습니다" |
| `M20_COMPLETE` | M20 카운트 완료 | "카운트가 완료되었습니다 (87/100)" |
| `OVERRIDE_CHANGE` | 오버라이드 변경 | "이송 오버라이드 120%로 변경" |
| `CONTROL_LOCK` | 제어권 획득 | "admin님이 제어권을 획득했습니다" |
| `CONTROL_UNLOCK` | 제어권 해제 | "admin님이 제어권을 해제했습니다" |
| `INTERLOCK_CHANGE` | 인터록 상태 변경 | "도어록이 해제되었습니다" |
| `COMMAND_SENT` | 명령 전송 | "명령 전송됨" |
| `COMMAND_ACK` | 명령 응답 | "명령 실행 완료" |

#### DNC 프로그램 경로 설정 (스케줄러 내 접힘/펼침 패널)

**목적**: DNC 모드 실행 시 각 Path별 NC 프로그램 소스 폴더를 사전 지정

**위치**: 인터록 바와 2분할 레이아웃(NC 모니터 + 테이블) 사이에 접힘/펼침 섹션으로 배치

```
┌─────────────────────────────────────────────────────────────────────────┐
│ ▶ DNC 프로그램 경로 설정                          [접힘 상태 기본]      │
├─────────────────────────────────────────────────────────────────────────┤
│ ▼ DNC 프로그램 경로 설정                          [펼침 상태]           │
│ ┌─────────────────────────────────────────────────────────────────────┐ │
│ │ Path 1:  /server/programs/MC-001/Path1/             [선택] [삭제]  │ │
│ │ Path 2:  /server/programs/MC-001/Path2/             [선택] [삭제]  │ │
│ │ Path 3:  (미설정)                                   [선택]         │ │
│ └─────────────────────────────────────────────────────────────────────┘ │
│ ※ ADMIN 전용, 스케줄러 실행 중 편집 불가                               │
└─────────────────────────────────────────────────────────────────────────┘
```

**데이터 구조**:
```typescript
interface DncPathConfig {
  path1: string;     // Path 1 프로그램 폴더 경로 (필수)
  path2: string;     // Path 2 프로그램 폴더 경로 (필수)
  path3?: string;    // Path 3 프로그램 폴더 경로 (3-Path 장비 전용)
}

interface MachineDncConfig {
  machineId: string;
  updatedAt: string;
  paths: DncPathConfig;
}
...
| 조건 | 동작 |
|------|------|
| ADMIN 권한 | 경로 선택/삭제 가능 |
| USER/AS 권한 | 읽기 전용 (선택 버튼 비활성) |
| 스케줄러 RUNNING | 전체 편집 불가 (인터록) |
| Path 수 결정 | 장비별 `pathCount` 필드 (2 또는 3) |

**폴더 브라우저**:
- 경로 "선택" 클릭 시 서버 파일시스템 탐색 모달 표시
- 폴더만 선택 가능 (파일 선택 불가)
- Mock 모드: `/server/programs/{machineId}/{Path1,Path2,Path3}/` 구조

**확장성**:
- 향후 DNC 모드 스케줄러 실행 시, 설정된 경로에서 프로그램을 자동 로드
- Path별 독립 경로 → 멀티 Path 장비의 독립 프로그램 관리 지원
...
NC 모니터는 스케줄러 좌측 패널에서 **탭 기반 UI**로 다양한 CNC 데이터를 표시:

```
┌─────────────────────────────────────────────┐
│ [모니터] [카메라] [옵셋] [카운터] [공구수명] │  ← 탭 전환
├─────────────────────────────────────────────┤
│                                             │
│  현재 탭에 해당하는 뷰 렌더링               │
│                                             │
...
| 탭 ID | 표시명 | 컴포넌트 | 설명 |
|--------|--------|----------|------|
| `monitor` | 모니터 | NCMonitor (내장) | 프로그램 표시, 좌표, 스핀들/이송 |
| `camera` | 카메라 | CameraStream | 카메라 ON + 장비 매핑 시에만 표시 |
| `offset` | 옵셋 | OffsetView | 공구보정값 (Wear/Geometry, Path1/Path2) |
| `count` | 카운터 | CountView | 생산카운터, 가동시간, 바피더 |
| `tool-life` | 공구수명 | ToolLifeView | 공구별 Preset/Count/잔여수명 |

#### 6.6.2 인라인 편집 패턴 (onBlur Commit)

옵셋, 카운터, 공구수명 뷰는 **저장 버튼 없이 onBlur 시점에 즉시 커밋**하는 인라인 편집 패턴을 사용:

```
사용자 셀 클릭 → 입력 모드 활성화 → 값 수정 → 포커스 이탈(onBlur)
                                                    ↓
                                              Store 업데이트 → API 전송 (향후)
```

**규칙**:
- 편집 권한: `user?.role === 'ADMIN' || user?.role === 'AS'`
- 숫자 입력만 허용 (offset, count 값)
- 변경 즉시 반영, 실패 시 원복
- 적용 대상: OffsetView, CountView, ToolLifeView
...
interface OffsetData {
  path1Wear: OffsetEntry[];      // Path1 마모 보정 (64개)
  path1Geometry: OffsetEntry[];  // Path1 형상 보정 (64개)
  path2Wear: OffsetEntry[];      // Path2 마모 보정
  path2Geometry: OffsetEntry[];  // Path2 형상 보정
}
...
#### 6.6.4 축 구성의 템플릿 연동 (향후)

현재 NC 모니터의 축 표시는 하드코딩:
```typescript
// NCMonitor.tsx (현재)
const axes = ['X', 'Y', 'Z', 'C'];  // 하드코딩
```

**향후 계획**: 템플릿의 `axisConfig.path1.axes`, `axisConfig.path2.axes`에서 동적으로 로드
```typescript
// 향후 적용 예정
const axes = template.axisConfig[pathKey].axes;  // ["X", "Z", "C", "Y", "A", "CRG"]
```
...
│ 프로그램 전송 [▼ 장비 선택]                                              │
├──────────────────────┬──────┬───────────────────────────────────────────┤
│ CNC 프로그램 (4.5fr) │  ↔  │ PC 프로그램 (5.5fr)                       │
│ ┌──────────────────┐ │      │ ┌───────────────────────────────────────┐ │
│ │[PATH1][PATH2]    │ │  →   │ │  제목 (동일 높이 h-10)               │ │
│ ├──────────────────┤ │  ←   │ ├───────────────────────────────────────┤ │
│ │ O0001  2.1KB ... │ │      │ │ O0001.NC  2.3KB ...                  │ │
│ │ O0002  1.8KB ... │ │      │ │ O0002.NC  1.5KB ...                  │ │
│ │ (flex-1 스크롤)  │ │      │ │ (flex-1 스크롤)                      │ │
│ ├──────────────────┤ │      │ ├───────────────────────────────────────┤ │
│ │ 전송 이력 (140px)│ │      │ │ 전송 이력 (140px)                    │ │
│ └──────────────────┘ │      │ └───────────────────────────────────────┘ │
├──────────────────────┴──────┴───────────────────────────────────────────┤
│ [백업] [복원] [시각]                                                     │
└──────────────────────────────────────────────────────────────────────────┘
...
**패널 배치 원칙**:
- **좌측 = CNC** (장비측), **우측 = PC** (서버측)
- 비율: `grid-cols-[4.5fr_auto_5.5fr]`
- 제목줄 높이 동일 (`h-10`) - PATH 탭은 CNC 제목줄 내 `headerSlot`에 삽입

#### 7.4.2 컴포넌트 구조

```
packages/web/src/
├── pages/Transfer.tsx              # 메인 페이지 (레이아웃 + 확인 다이얼로그)
├── stores/fileStore.ts             # 파일 상태관리 (Zustand)
└── components/filemanager/
    ├── FileListPanel.tsx           # 재사용 파일 목록 패널
    ├── TransferArrows.tsx          # 전송 방향 화살표 UI
    ├── TransferQueuePanel.tsx      # 전송 이력/큐 패널
    ├── GCodeViewer.tsx             # GCode 모달 뷰어
    ├── RepositoryBrowser.tsx       # DNC 레포지토리 브라우저
    └── mockFileData.ts             # Mock 데이터
```

#### 7.4.3 FileStore 상태 구조

```typescript
interface FileStore {
  // ── 파일 목록 (Path-aware) ──
  repoFiles: Record<string, FileEntry[]>;  // 키: "machineId:pathKey"
  cncFiles: Record<string, FileEntry[]>;   // 키: "machineId:pathKey"
  pcFiles: FileEntry[];
...
interface FileEntry {
  ...
  programNo?: string;     // "O0001" 등 (파일명에서 추출)
  comment?: string;       // NC 프로그램 설명
}
```

#### 7.4.4 Path-aware 파일 로딩

CNC 파일은 **장비ID + Path키** 조합으로 스코프됨:

```
loadCncFiles("MC-001", "path1")
  → mockData["MC-001:path1"] 조회
  → state.cncFiles["MC-001:path1"] = [...]
```

**PATH 탭 UI**: CNC FileListPanel의 `headerSlot`에 PATH1/PATH2/PATH3 버튼 삽입
- PATH 전환 시 해당 path의 파일 목록 로드
- Path3은 3계통 장비에서만 표시 (`machine.pathCount >= 3`)

#### 7.4.5 전송 확인 다이얼로그

전송 실행 전 **확인 모달**을 표시하여 오조작 방지:

```
[확인 다이얼로그]
┌──────────────────────────────────┐
│  PC → CNC 전송                   │  ← 방향 표시 (색상 구분)
│                                  │
│  · O0001.NC                     │  ← 선택된 파일 목록
│  · O0002.NC                     │
│  · O0003.NC                     │
...
- 방향: `PC→CNC` (파란색), `CNC→PC` (초록색)
- 권한: `ADMIN` 또는 `AS` 역할만 전송 가능

#### 7.4.6 전송 이력 관리 (FIFO)

```typescript
const TRANSFER_HISTORY_MAX = 100;  // 최대 이력 보관 수
...
**FIFO 정책**: 이력이 100건을 초과하면 가장 오래된 항목부터 자동 삭제
**삭제 권한**: 이력 수동 삭제는 `ADMIN` 역할만 가능
**UI**: 고정 높이 140px, 내부 스크롤, 항상 표시 (빈 상태에서도)

#### 7.4.7 다크 테마

Transfer 페이지 전체에 다크 테마 적용:
- 배경: `bg-gray-900` (NCMonitor와 동일)
- 파일 목록: `bg-gray-800`, hover시 `bg-gray-700`
- 전송 큐: `bg-gray-850` 계열
- 백업 영역도 동일 다크 테마
...
#### 9.1.4 IP 카메라 연동 (선택적 기능)

**목적**: CNC 장비의 가공 상태를 IP 카메라(RTSP)로 실시간 모니터링 (전시회 데모 등)

**핵심 원칙**:
- **완전 분리 설계**: CNC 핵심 기능(모니터링/제어/스케줄러)과 완전히 독립
- **선택적 활성화**: 시스템 설정에서 ON/OFF 토글로 전체 카메라 서브시스템 제어
- **카메라 OFF 시**: 관련 UI 요소 전체 숨김 (NC 모니터 카메라 탭 포함)

**대상 장비**: Hikvision IP 카메라 (RTSP over LAN)
- 프로토콜: RTSP → WebRTC 변환 (저지연 < 1초 목표)
- 네트워크: 로컬 LAN 전용 (인터넷 불필요)

**카메라 설정 데이터 구조**:
```typescript
interface CameraConfig {
  id: string;
  name: string;           // 카메라 이름 (예: "CAM-1호기")
  ipAddress: string;      // IP 주소
  rtspPort: number;       // RTSP 포트 (기본: 554)
  username: string;       // 접속 ID
  password: string;       // 접속 PW (base64 인코딩 저장)
  streamPath: string;     // RTSP 스트림 경로
  enabled: boolean;       // 개별 카메라 활성화 여부
  assignedMachineId?: string;  // 매핑된 장비 ID (1:1)
}
```
...
| 위치 | 표시 조건 | 내용 |
|------|-----------|------|
| **Settings 페이지** | 항상 | 카메라 ON/OFF 토글, 카메라 CRUD 테이블 |
| **NC 모니터 (카메라 탭)** | 카메라 ON + 장비에 카메라 매핑됨 | 해당 장비 카메라 실시간 스트리밍 |
| **NC 모니터 (카메라 탭)** | 카메라 OFF 또는 미매핑 | "연결된 카메라 없음" 메시지 |

**관리 권한**:
- 카메라 ON/OFF 토글: ADMIN 전용
- 카메라 추가/수정/삭제: ADMIN 전용
- 카메라 스트리밍 조회: 모든 사용자

**저장소**: localStorage (키: `star-webcnc-camera-config`)
- Mock 모드 기본 제공 (백엔드 없이 UI 테스트 가능)
- 향후 서버 API 연동 시 전환
...
┌──────────────┐     RTSP      ┌───────────────┐     WebRTC     ┌──────────────┐
│  IP Camera   │──────────────▶│  Media Server │──────────────▶│  Web Browser │
│  (Hikvision) │   LAN Only    │  (go2rtc 등)  │   Low Latency  │  (Frontend)  │
└──────────────┘               └───────────────┘                └──────────────┘
       │                              │
       └── 로컬 네트워크 전용 ────────┘
           (인터넷 불필요)
...
│  Phase 4+ [확장 기능]    ████████████████████████████████████████  100% ✅      │
│    - Mock 데이터 보강 (POP/WorkOrder/Alarms/AuditLog)                          │
│    - DNC 프로그램 경로 설정 (장비별 Path 1/2/3)                                │
│    - IP 카메라 연동 옵션 (Settings + NCMonitor 카메라 탭)                       │
│  Phase 4++ [UI 고도화]   ████████████████████████████████████████  100% ✅      │
│    - Transfer 다크테마 + 패널 재배치 (좌=CNC, 우=PC)                           │
│    - FileManager 컴포넌트 (FileListPanel, TransferQueue, GCodeViewer)          │
│    - Path-aware 파일 로딩 (machineId:pathKey 스코프)                           │
│    - 전송 확인 다이얼로그 + FIFO 이력 관리 (100건)                              │
│    - NC Monitor 탭 시스템 (옵셋/카운터/공구수명 인라인 편집)                    │
│    - 장비 템플릿 예제 양식 작성 (templates/template_example.jsonc)              │
│  Phase 5 [실장비 연동]   █████████████████████████████░░░░░░░░░░  70% ⏳      │
│    ✅ MachineTopBar Row2 2분할 + 템플릿 기반 인터록 (pageId)                   │
│    ✅ WebSocket 클라이언트 실연결 (wsClient.ts + 자동 재연결)                   │
│    ✅ machineStore.fetchMachines REST API + controlLockMap 복원                 │
│    ✅ fileStore 5개 액션 API 연결 (API 우선 → Mock fallback)                   │
│    ✅ MachineTopBar controlLock 서버 API 연동 (acquire/release/extend)          │
│    ✅ 서버 /api/files 라우트 구현 (repo/share 저장소, 파일 전송 MQTT)           │
│    ✅ Agent FOCAS 명령 구현 (LIST_PROGRAMS / UPLOAD_PROGRAM / DOWNLOAD_PROGRAM) │
│       - FocasDataReader: ListPrograms, UploadProgramAsync, DownloadProgramAsync │
│       - CommandHandler: 3개 명령 케이스 추가                                   │
│       - 기존 FOCAS2 API 필드명 버그 3건 수정 (ODBST/ODBALMMSG2/ODBM)          │
│    ✅ Server MQTT AGENT_COMMAND_RESULT 핸들러                                  │
│       - DOWNLOAD_PROGRAM 결과 → data/share/ 자동 저장                          │
│       - WebSocket command_result / file_downloaded 이벤트 브로드캐스트          │
│    ✅ Server 명령 응답 대기 (commandWaiter — in-memory Promise 레지스트리)      │
│       - POST /api/commands/:id?wait=true → Agent 결과까지 동기 블로킹 (30s)    │
│       - DB CommandLog SUCCESS/FAILURE 자동 업데이트                             │
│       - machineStore WS handler: command_result / file_downloaded 이벤트       │
│       - commandApi.sendAndWait() 프론트엔드 API 추가                            │
│    ⏳ 통합 테스트 / 실장비 연동 검증                                           │
│                                                                                 │
│  ✅ 완료  ⏳ 진행 예정  🔒 대기 중                                              │
└─────────────────────────────────────────────────────────────────────────────────┘
...
### v3.7 (2026-02-09) - UI 고도화 및 아키텍처 정비
**추가된 섹션**:
- [SEC 1.7] UI 테마 및 공통 패턴 (다크 테마, 공장 레이아웃 편집, 역할 기반 UI 제어)
- [SEC 2.6] Frontend 상태관리 패턴 (Zustand Store 구성, Custom Selector Hook, Mock 전략)
- [SEC 6.6] NC Monitor 탭 시스템 (5-탭 구성, 인라인 편집 onBlur 패턴, 옵셋 데이터 구조, 축 템플릿 연동 계획)
- [SEC 7.4] Transfer UI 아키텍처 (FileStore 구조, Path-aware 파일 로딩, 전송 확인 다이얼로그, FIFO 이력, 다크 테마)

**업데이트된 내용**:
- [ROADMAP] Phase 4++ (UI 고도화) 추가, Phase 5 (실장비 연동) 진행률 업데이트
- [TEMPLATE] 장비 템플릿 예제 양식 작성 (`templates/template_example.jsonc`)
  - SB-20R2 (FANUC 0i-TF Plus) 기본 정보 기입 완료
  - 축 구성: Path1 [X,Z,C,Y,A,CRG], Path2 [X,Z,C,A]
  - PMC 주소, 가상 조작반, 스케줄러 설정은 현장 확인 후 기입 예정

**구현 완료 항목 (코드)**:
- Transfer 페이지 전면 재구성: 다크테마, 패널 스왑(좌=CNC,우=PC), Path 탭
- FileManager 컴포넌트: FileListPanel, TransferArrows, TransferQueuePanel, GCodeViewer
- fileStore.ts: Path-aware 로딩, FIFO 이력 관리, TransferJob 인터페이스 확장
- 전송 확인 다이얼로그 (TransferConfirmDialog)

### v3.6 (2026-02-04) - IP 카메라 연동 (선택적 기능)
**추가된 내용**:
- [CAMERA] IP 카메라 연동 옵션 (완전 분리 설계)
  - cameraStore.ts: Zustand + localStorage 기반 카메라 설정 관리
  - CameraStream.tsx: 실시간 스트리밍 뷰어 (Mock 모드 + WebRTC 준비)
  - Settings.tsx: 시스템 설정 페이지 신규 생성
  - 카메라 ON/OFF 전역 토글 (ADMIN 전용)
  - 카메라 CRUD (이름, IP, RTSP 포트, 인증정보, 스트림 경로, 장비 매핑)
  - NCMonitor 카메라 탭 연동 (장비별 매핑된 카메라 스트리밍)
  - Hikvision RTSP → WebRTC 저지연 스트리밍 아키텍처 정의
  - 로컬 LAN 전용 운영 (인터넷 불필요)
- [MENU] Settings 메뉴 추가 (좌측 사이드바, 기어 아이콘)
- [ROUTE] /settings 라우트 추가
- [API] cameraApi 정의 (getAll, create, update, delete, getWebRTCOffer, getStatus)

### v3.5 (2026-02-04) - DNC 프로그램 경로 설정
**추가된 내용**:
- [DNC] 장비별 DNC 프로그램 경로 설정 기능
  - DncPathConfig / MachineDncConfig 인터페이스 (machineStore)
  - Machine 인터페이스에 pathCount 필드 추가 (2 또는 3 Path)
  - Scheduler.tsx: DNC 경로 설정 접힘/펼침 패널 (인터록 바 아래)
  - FolderBrowser.tsx: 서버 폴더 탐색 모달 (Mock 폴더 트리 포함)
  - ADMIN 전용 편집, 스케줄러 RUNNING 시 편집 잠금
  - localStorage 영속화 (키: star-webcnc-dnc-config)
- [API] dncApi 정의 (listFolders, getConfig, saveConfig)
- [STORE] machineStore에 dncConfigs 상태 및 setDncConfig 액션 추가

### v3.4 (2026-02-04) - UI 페이지 Mock 데이터 보강
**추가된 내용**:
- [MOCK] POP 페이지: 4개 장비 생산 통계 + 7일간 차트 데이터
- [MOCK] WorkOrder 페이지: 6건 작업지시 (PENDING/IN_PROGRESS/COMPLETED/CANCELLED)
- [MOCK] Alarms 페이지: 8건 알람 기록 (활성 3건, 해제 5건)
- [MOCK] AuditLog 페이지: 12건 감사 로그 (제어/스케줄러/전송/인증/관리 액션)
- API 호출 실패 시 Mock 데이터 폴백 패턴 적용 (백엔드 없이 UI 프리뷰)
...
### v3.3 (2026-01-28) - 스케줄러 UI 전면 재구성
**변경 내용**:
- [UI] 스케줄러 화면 전면 재구성
  - 헤더: 제목 + 장비선택 + 제어권획득 버튼 + 상태 메시지
  - 인터록 바: 도어록, 메모리모드, 바피더오토, 절삭유ON, 머시닝모드, 절단모드, 기타(2)
  - NC 모니터: Path1/Path2 프로그램 내용(6줄), 좌표계(절대/기계/잔량), 스핀들/이송
  - 스케줄러 테이블: 현재실행 + 작업큐 (메인PGM, 서브PGM, PRESET, COUNT, 상태, 삭제)
  - FOCAS 이벤트 로그: 원격 명령/상태 피드백 실시간 모니터링
- [STORE] machineStore 확장
  - PathData 인터페이스 (Path1/Path2 프로그램, 좌표, 스핀들/이송)
  - InterlockStatus 인터페이스 (인터록 상태)
  - FocasEvent 인터페이스 (이벤트 로그)
  - useFocasEvents 셀렉터 추가
- [DATA] TelemetryData 확장
  - path1, path2 필드 추가 (PathData)
  - interlock 필드 추가 (InterlockStatus)
- [UX] 제어권 상태 표시 개선
  - "제어권 획득이 가능합니다"
  - "인터록 조건이 만족되지 않았습니다"
  - "누구누구님이 사용중입니다"

### v3.2 (2026-01-28) - 장비 선택기 위치 변경
**변경 내용**:
- [UI] 장비 선택기를 사이드바에서 페이지 헤더로 이동
  - Layout.tsx: 사이드바에서 장비 선택기 섹션 제거
  - RemoteControl.tsx: 페이지 헤더에 장비 선택 드롭다운 추가 (제목 옆)
  - Scheduler.tsx: 페이지 헤더에 장비 선택 드롭다운 추가 + 작업 필터링
  - Transfer.tsx: 페이지 헤더에 장비 선택 드롭다운 통합
- [UX] Remote Panel 장비 미선택 시 동작 개선
  - 장비 미선택 시 MachineCard 목록 표시
  - 헤더 드롭다운 또는 카드 클릭으로 장비 선택 가능
- [UX] Scheduler 장비 필터링 기능
  - "전체 장비" 선택 시 모든 작업 표시
  - 특정 장비 선택 시 해당 장비의 작업만 필터링
- [STORE] selectedMachineId를 Machines 하위 페이지에서 공유
  - 한 페이지에서 장비 선택 시 다른 페이지에도 자동 반영

### v3.1 (2026-01-28) - 메뉴 구조 개편
**변경 내용**:
- [MENU] Machines 하위 메뉴 구조 적용
  - Dashboard
  - Machines ▸ (펼침/접힘 메뉴)
    - Remote Panel (/remote)
    - Scheduler (/scheduler)
  ...
  - Alarms
  - Audit Log
- [UI] 하위 메뉴 펼침/접힘 상태 관리 (expandedMenus)
- [ROUTE] /machines 라우트 제거 - Machines 페이지 삭제
- [ROUTE] /remote/:machineId → /remote 변경
  - URL 파라미터 대신 store의 selectedMachineId 활용
- [CODE] Machines.tsx 삭제 (불필요한 페이지 제거)
- [ICON] ChevronIcon, RemotePanelIcon 추가

### v3.0 (2026-01-28) - Dashboard 개선
**변경 내용**:
- [CardView] 카드뷰 컨텐츠 개선
  - 호기명 / 장비명
  - 가동상태 (색깔)
  - PRESET / COUNT (목표량 및 카운트량) + 진행률 바
  - 일일 가동률 (00시 기준)
  - 사이클타임
  - 메인/서브 프로그램명, 제품명 (NC 코멘트)
- [FactoryView] 공장 레이아웃 개선
  - 설비 클릭 시 카드뷰 내용 팝업 모달 표시
  - 레이아웃 크기 조절 설정 (너비/높이)
  - 통로(corridor) 및 출입구(door) 편집 가능
- [Store] layoutStore 확장
  - corridor, door 타입 추가
  - resizeLayout() 함수 추가
- [Utils] 공용 유틸리티 생성
  - lib/constants.ts: STATUS_COLORS, ZOOM, WS_RECONNECT_DELAY
  - lib/types.ts: MachineStatus, MachineStatsSummary
  - lib/machineUtils.ts: getStatusColor, getStatusText, getRunStateText 등
...
| 버전 | 날짜 | 변경 내용 |
|------|:---:|-----------|
| 3.6 | 2026-02-04 | **IP 카메라 연동 (선택적 기능)** <br> - CameraStore, CameraStream, Settings 페이지 추가 <br> - NC 모니터 카메라 탭 연동 <br> - Hikvision RTSP → WebRTC 아키텍처 <br> - Settings 메뉴 및 라우트 추가 |
| 3.5 | 2026-02-04 | **DNC 프로그램 경로 설정** <br> - 장비별 Path 1/2/3 DNC 경로 설정 UI <br> - 폴더 브라우저 모달 <br> - ADMIN 전용 + 스케줄러 인터록 |
| 3.4 | 2026-02-04 | **UI 페이지 Mock 데이터** <br> - POP/WorkOrder/Alarms/AuditLog Mock 데이터 추가 <br> - API 실패 시 Mock 폴백 패턴 |
...
| 3.1 | 2026-01-28 | **메뉴 구조 개편** <br> - Machines 하위 메뉴 구조 적용 (펼침/접힘) <br> - Remote Panel, Scheduler, File Transfer 하위 메뉴화 <br> - Machines.tsx 페이지 삭제 <br> - /remote/:machineId → /remote 라우트 변경 |
| 3.2 | 2026-01-28 | **장비 선택기 위치 변경** <br> - 사이드바에서 장비 선택기 제거 <br> - Machines 하위 페이지 헤더에 장비 선택기 추가 <br> - Remote Panel: 헤더 + 카드 선택 <br> - Scheduler: 헤더 + 작업 필터링 <br> - Transfer: 헤더 통합 |
| 3.3 | 2026-01-28 | **스케줄러 UI 전면 재구성** <br> - Path1/Path2 NC 모니터 (프로그램 내용, 좌표계, 스핀들/이송) <br> - 인터록 바 (도어록, 메모리모드, 바피더오토 등 8개 항목) <br> - FOCAS 이벤트 로그 (원격 명령/상태 피드백) <br> - 제어권 상태 메시지 개선 |
