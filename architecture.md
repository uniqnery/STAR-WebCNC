...
### 1.1 프로젝트 목적
FANUC CNC 자동선반(Bar Feeder 연동)을 웹 기반으로 모니터링하고 제한적 원격 제어를 수행하는 스마트팩토리 시스템 구축
...
│ │  STAR   │ ├─────────────────────────────────────────────────────┤ │
│ │  로고   │ │  메인: 공장 레이아웃 (2D / 3D 선택)                │ │
│ │    +    │ │       · 장비 상태 색상 표시 (가동/대기/알람/오프)  │ │
│ │ 메뉴바  │ │       · 실시간 가동 현황 한눈에 확인               │ │
│ │         │ │       · 장비 클릭 → 상세 페이지 이동               │ │
│ └─────────┘ └─────────────────────────────────────────────────────┘ │
└─────────────────────────────────────────────────────────────────────┘
     │
...
interface EquipmentTemplate {
  ...
  pmcMap: {
    ...
    interlock: {
      ...
      barFeederReady: PMCAddress;    // Bar Feeder 준비
...
**목적**: 프로그램 순차 실행 자동화 (Bar Feeder 연동 자동선반)
...
```
┌─────────────────────────────────────────────────────────────────────┐
│ [도어] [척] [스핀들] [Bar Feeder] [쿨런트]  ← 인터록 상태 바 (상단) │
├─────────────────────────────┬───────────────────────────────────────┤
│                             │                                       │
│  FOCAS 실시간 데이터        │  스케줄러 테이블                      │
│  ─────────────────────      │  ─────────────────────                │
│  좌표계 (절대/기계/상대)    │  Main PGM | Sub PGM | PRESET | COUNT │
│  스핀들 RPM/부하            │  ─────────────────────────────────────│
│  이송 속도                  │  O0001   | O0100   | 50     | 0/100  │
│  모달 정보                  │  O0002   | O0101   | 30     | 0/50   │
│  실행 프로그램명            │  O0003   | -       | 0      | 0/200  │
│                             │  ...                                  │
│                             │  ─────────────────────────────────────│
│  [카메라 화면 영역]         │  [시작] [정지] [리셋] [삭제]          │
│  (향후 확장용)              │                                       │
│                             │                                       │
└─────────────────────────────┴───────────────────────────────────────┘
```
...
| 컬럼 | 설명 |
|------|------|
| Main PGM | 메인 가공 프로그램 번호 (예: O0001) |
| Sub PGM | 서브 프로그램 번호 (옵션) |
| PRESET | 프리셋 수량 (목표 생산량) |
| COUNT | 현재 수량 / 목표 수량 (실시간 갱신) |
| Status | 대기/실행중/완료/오류 |
| Delete | 삭제 버튼 (항상 표시, 클릭 시 확인 팝업) |
...
└─────────────────────────────────────────────────────────────────────┘
     ...
     [사이클 완료]        [인터록 위반]          [정지 요청]
           ...
           ▼                    ▼                    ▼
┌─────────────────┐  ┌─────────────────────┐  ┌─────────────────────┐
│ 4. COUNT 증가   │  │ 자동 정지           │  │ 1사이클 완료 후 정지│
│ PRESET 도달?    │  │ - Feed Hold 전송    │  │ (수동 중지)         │
└─────────────────┘  │ - 알람 기록         │  └─────────────────────┘
      │      │       │ - 사용자 알림       │
   [Yes]  [No]       └─────────────────────┘
      │      │
...
// 인터록 검증 결과 예시
{
  ...
  "checks": [
    ...
    { "condition": "barFeederReady", "required": true,  "currentValue": true,  "passed": true  }
  ],
...
interface AuditLog {
  ...
  action: string;              // 예: "scheduler.start", "transfer.upload"
...
| 1.1 | 2026-01-22 | 아키텍처 리뷰 반영 |
| | | - 인터록 설정을 Template 전용으로 변경 (섹션 3.3, 4.2) |
| | | - Agent 로컬 캐싱 전략 추가 (섹션 2.2) |
| | | - 스케줄러/Transfer 동시 실행 가능으로 변경 (섹션 7.1) |
| | | - 메시지 스키마 및 API 표준 추가 (섹션 2.4) |
| | | - 복원 정책 추가: 원격 불가 (섹션 7.2.5) |
| 1.2 | 2026-01-22 | 메뉴 접속도 추가 |
| | | - 메뉴 접속도 다이어그램 추가 (섹션 1.5) |
| | | - 역할별 메뉴 접근 권한 요약 추가 (섹션 1.6) |
