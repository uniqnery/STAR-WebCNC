...
MachineTopBar를 실제 사용 — `rightSlot`으로 SIMTOS 전용 인터록 pills 삽입.
...
- `telemetry.mode`, `telemetry.runState` 기반 인터록 설계는 폐기
...
- `MachineTopBar`의 `lockDisabled={!interlockSatisfied}` — 인터록 미충족 시 제어권 버튼 비활성화
...
> **사이클 스타트 5회 루프**: OCS ON 상태에서 첫 M20 정지 후 재스타트 자동 처리 목적.
...
| # | 항목 | 내용 |
|---|------|------|
| 1 | 이미지 파일 | 실제 전시용 이미지로 교체 필요 (현재 placeholder) |
| 2 | 사이클 스타트 가동 확인 | 현재 5회 루프 후 runState 확인. 가동 전 확인으로 개선 가능 |
